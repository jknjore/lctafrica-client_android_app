package com.lctapp.lct.Classes.Helpers

object AppConstants {
    const val GPS_REQUEST = 1001
    const val LOCATION_REQUEST = 1000
    const val COUNTRY_REQUEST = 2
    const val BASE_URL = "http://35.241.171.182:8085"
    const val START_VISIT_URL ="http://35.241.171.182:8085/compas/rest/user/insertVerifiedMember"
    const val START_VISIT_NGROK ="http://7f1bd6d5ec8a.ngrok.io/compas/rest/user/insertVerifiedMember"
    const val VALIDATE_LOCATION_URL ="http://35.241.171.182:8085/compas/rest/user/validateUserMobileAppLocation"
    const val NGROK_API_ = "http://7f1bd6d5ec8a.ngrok.io/compas/rest/user/validateUserMobileAppLocation/"


    const val START_VISIT_DEVICEID = "mobileApp"
}